...
v1.2 Updates:
stopwatch
	Made stopwatch into a class
	Added Laps to stopwatch
	Removed milliseconds
		Unnecessary and caused the terminal to flicker distractingly
...
Fixed usage print out to automatically change depending on OS

Known issues:
	Adding a lap in stopwatch or changing color in clock takes a second to display after hitting Enter
	This is due to the terminal only refreshing once per second
	Refreshing any further would cause flickering
	May fix if stopwatch and clock are changed to use a proper UI instead of the terminal
...
Use proper UI for stopwatch and clock functions
"""
